...
- Keep responses under 150 words unless detailed info is needed.`;
  ...
  private async *generateGeminiResponseStream({
    ...
    // Important: Gemini streaming responses can vary by model/key and sometimes
    // return JSON arrays (not SSE). To keep the UX reliable for all students,
    // we always generate a complete response first, then stream it to the UI in chunks.
    ...
    for (const chunk of chunkText(full, 28)) yield chunk;
  }
